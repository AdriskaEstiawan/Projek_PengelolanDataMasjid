...
if (isset($_POST['saveJ'])) {
    ...
    $query = "INSERT INTO jadwal (waktu, tanggal, penceramah, hari, kajian) VALUES ('$waktu', '$tgl', '$ceramah', '$hari', '$kajian')";

    if ($conn->query($query) === TRUE) {
        echo "<script>
                alert('Jadwal Kajian Berhasil Ditambahkan');
                window.location.href='home1.php?page=jadwal';
              </script>";
    } else {
        echo "<script>
    ...
    <div class="row">
        <div class="col-12">
            <form action="" method="POST">
                ...
                <select name="ceramah" required>
                    <option value="">--Pilih--</option>
                    <?php
                    $sql = $conn->query("SELECT id_kajian, nm_ustad FROM kajian");
                    while ($data = $sql->fetch_assoc()) {
                        echo "<option value='{$data['id_kajian']}'>{$data['nm_ustad']}</option>";
                    }
                    ?>
                </select>
                ...
                <label>Kajian</label>
                <select name="kajian" required>
                    <option value="">--Pilih--</option>
                    <?php
                    $sql2 = $conn->query("SELECT id_kajian, nm_kajian FROM kajian");
                    while ($data2 = $sql2->fetch_assoc()) {
                        echo "<option value='{$data2['id_kajian']}'>{$data2['nm_kajian']}</option>";
                    }
                    ?>
                </select>
